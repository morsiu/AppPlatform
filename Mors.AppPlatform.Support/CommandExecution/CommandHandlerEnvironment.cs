...
namespace Mors.AppPlatform.Support.CommandExecution
{
    ...
    public sealed class CommandHandlerEnvironment
    {
        ...
        public bool StoreQueuedEvents()
        {
            ...
            {
                _logger.LogStoreOfEventsGeneratedByCommandHandlerFailed(exception);
                return false;
            }
        }
        ...
        private bool DispatchQueuedEvents()
        {
            ...
            foreach (var queuedEvent in _queuedEvents)
            {
                ...
                {
                    _logger.DispatchOfEventQueuedByCommandHandlerFailed(exception, queuedEvent);
        ...
        private bool StoreAggregates()
        {
            foreach (var aggregateEntry in _storedAggregates)
            {
                ...
                {
                    _logger.LogStoreOfAggregateStoredByCommandHandlerFailed(exception, aggregate, aggregateId, aggregateType);
                    return false;
                }
            }
            return true;
        ...
        private sealed class Logger
        {
            ...
            internal void LogStoreOfEventsGeneratedByCommandHandlerFailed(Exception exception)
            {
                _logger(new { Origin = "CommandHandlerEnvironment", Event = "Store of events generated by handler failed", Exception = exception });
            }

            internal void LogStoreOfAggregateStoredByCommandHandlerFailed(Exception exception, Aggregate aggregate, AggregateId aggregateId, AggregateType aggregateType)
            {
            ...
            internal void DispatchOfEventQueuedByCommandHandlerFailed(Exception exception, Event queuedEvent)
            {
